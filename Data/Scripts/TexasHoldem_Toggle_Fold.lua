...
function UpdateCallText(data)
    if (data.players[LOCAL_PLAYER.name] or {}) and 
    (data.players[LOCAL_PLAYER.name] or {}).state == TexasPoker_Enums.PlayerStates.Fold then
        return
    end

    --[[
    if data.roundBet == (data.players[LOCAL_PLAYER.name] or {}).roundBet or data.roundBet == 0 then
        CallText.text = 'Check/Fold'
...
CheckBox.changedEvent:Connect(
    function(value)
        ...
        if value and oldData and (oldData.players[LOCAL_PLAYER.name] or {}).slotNumber == oldData.turn then
            if (oldData.roundBet or 0) == 0 then
                TexasPoker_APIClient.Call()
            else
                TexasPoker_APIClient.Fold()
            end
