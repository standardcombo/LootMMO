...
local CallText = script:GetCustomProperty('CallText'):WaitForObject()
...
function UpdateCallText(data)
    if( data.players[LOCAL_PLAYER.name] or {}) and( data.players[LOCAL_PLAYER.name] or {}).state == TexasPoker_Enums.PlayerStates.Fold then
        return
    end
    ...
    if (data.roundBet or 0) == 0 then
        ...
        local currentbet =( data.players[LOCAL_PLAYER.name] or {}).roundBet or 0
        CallText.text = string.format('Call %d', data.roundBet - currentbet)
    end
    ]]
...
function CheckCall(player, action, data)
    if action == TexasPoker_Enums.playerActions.Raise then
        CheckBox:SetToggle(false)
    end
end
local function ToggleUncheck(Box)
    if Box == CheckBox then return end 
    Uncheck()
end 

local TP_Events = {
    TP_Client.updateEvent:Connect(UpdateCallText),
    TP_Client.playerActionChangedEvent:Connect(CheckCall),
